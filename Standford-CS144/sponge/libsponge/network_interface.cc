...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    // find ARP table first
    const auto &arp_iter = _arp_table.find(next_hop_ip);

    // APR table doesn't have this entry for MAC address
    if (arp_iter == _arp_table.end()) {
        ...
        if (_waiting_arp_response_ip_addr.find(next_hop_ip) == _waiting_arp_response_ip_addr.end()) {
            ...
            arp_request.target_ethernet_address = {/* empty */};
            arp_request.target_ip_address = next_hop_ip;

            EthernetFrame eth_frame;
            eth_frame.header() = {/* dest */ ETHERNET_BROADCAST,
        ...
        // put the IP packet into the waiting queue
        _waiting_arp_internet_datagrams.push_back({next_hop, dgram});
    } else {
        // send the ethernet frame and send;
        EthernetFrame eth_frame;
        eth_frame.header() = {/* dest */ arp_iter->second.eth_addr,
                              /* src  */ _ethernet_address,
...
optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    // filter out the packet not send to current address
    ...
    if (frame.header().type == EthernetHeader::TYPE_IPv4) {
        ...
        // not judge based on IP because here is link layer
        return datagram;
